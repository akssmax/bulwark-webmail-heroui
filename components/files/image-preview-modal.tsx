...
export function ImagePreviewModal({ name, onClose, onDownload, getImageUrl }: ImagePreviewModalProps) {
  ...
  return (
    <AppModal
      ...
      header={(
        <div
          className="flex items-center justify-between pl-[calc(1rem+env(safe-area-inset-left))] pr-[calc(1rem+env(safe-area-inset-right))] pt-[calc(0.75rem+env(safe-area-inset-top))] pb-3 bg-gradient-to-b from-black/60 to-transparent"
        ...
        >
          ...
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:bg-white/20" onClick={onClose} aria-label={t("cancel")}>
              <X className="w-4 h-4" />
            </Button>
          </div>
        </div>
      )}
      bodyClassName="flex items-center justify-center w-full h-full p-16 pt-[calc(4rem+env(safe-area-inset-top))] pb-[calc(4rem+env(safe-area-inset-bottom))]"
    >
      {loading && (
